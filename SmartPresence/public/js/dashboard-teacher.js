...
function updateDashboardUI(students, subjects, schedules, present, absent, late, logs) {
  ...
  const avgPercentage = totalRecords > 0 ? Math.round((present / totalRecords) * 100) : 0;

  if (totalStudentsEl) totalStudentsEl.textContent = students;
  if (totalSubjectsEl) totalSubjectsEl.textContent = subjects;
  if (totalSchedulesEl) totalSchedulesEl.textContent = schedules;
  if (avgAttendanceEl) avgAttendanceEl.textContent = avgPercentage + "%";

  // Fallback selectors for old grid structure (compatibility)
  ...
  if (orangeEl) orangeEl.textContent = schedules;

  // Update attendance overview (in analytics summary)
...
function displayAllLogsModal(logs) {
  ...
  if (logs.length === 0) {
    allLogsTableBody.innerHTML = `<tr><td colspan="5" style="text-align:center;">No attendance logs yet. Start marking attendance!</td></tr>`;
    updateLogsPageInfo();
    return;
  }
  ...
  if (prevLogsBtn) {
    prevLogsBtn.addEventListener("click", () => {
      if (currentLogsPage > 1) {
        currentLogsPage--;
        const sortedLogs = [...allLogsData].sort((a, b) => b.date.localeCompare(a.date));
        renderLogsPage(sortedLogs);
      }
    });
  }
