...
// ✅ CHECK QR DATA FIRST - Before authentication
if (qrDataParam) {
  try {
    const decodedData = decodeURIComponent(qrDataParam);
    const qrData = JSON.parse(decodedData);
    
    // Store QR data in sessionStorage for after login
    sessionStorage.setItem('pendingQRData', qrDataParam);
    ...
    showError('Invalid QR Code', 'Could not read QR code data. Please try again.');
  }
} else {
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (!user) {
    ...
    // ✅ FIXED: Redirect to index.html instead of login.html
  ...
  // ✅ Get QR data from sessionStorage or URL
  const storedQRData = sessionStorage.getItem('pendingQRData') || qrDataParam;
  
  if (!storedQRData) {
    console.error('❌ No QR data found');
    showError('Invalid QR Code', 'No attendance data found. Please scan a valid QR code.');
    return;
  }
  
  console.log('🔍 QR Data received:', storedQRData);
  
  // Parse QR data
  let qrData;
  try {
    const decodedData = decodeURIComponent(storedQRData);
    qrData = JSON.parse(decodedData);
  } catch (parseError) {
    console.error('❌ QR Parse error:', parseError);
    showError('Invalid QR Code', 'Could not parse QR code data.');
    return;
  }
  
  // ✅ Check if Face Recognition mode
  if (qrData.mode === 'faceRecognition') {
    console.log('🎭 Face Recognition QR detected - Checking verification status...');
    ...
    if (!isVerified) {
      console.log('❌ User not verified, redirecting to profile...');
      sessionStorage.setItem('verificationMessage', 'Please verify your account to use face recognition attendance.');
      sessionStorage.setItem('pendingReturnUrl', window.location.href);
      window.location.href = 'profile.html';
      return;
    }
    
    console.log('✅ User verified, redirecting to face recognition...');
    // Clear the pending QR data
    sessionStorage.removeItem('pendingQRData');
    // Redirect to face recognition page
    window.location.href = `face-recognition.html?data=${storedQRData}`;
    return;
  }
...
// Process attendance automatically (ONLY for automatic mode)
...
function showError(title, message) {
  ...
  actionButton.style.display = 'inline-block';
  // ✅ FIXED: Redirect to qr-scanner-student.html instead of trying to scan again
