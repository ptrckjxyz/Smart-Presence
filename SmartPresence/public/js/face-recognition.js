// js/face-recognition.js - FIXED CLASS NOT FOUND ERROR
...
async function initTeacherMode() {
    ...
    // ✅ FIX: Map URL department codes to Firebase department codes
    const departmentMap = {
        'IT': 'ITD',
        'ITD': 'ITD',
        'CS': 'CSD',
    ...
    if (snapshot.exists()) {
        ...
        // ✅ Try to help debug by showing what exists
...
async function loadVerifiedStudents() {
    try {
        ...
        if (userMode === 'teacher') {
            ...
            // ✅ FIX: Use the same department mapping
            const departmentMap = {
                'IT': 'ITD',
                'ITD': 'ITD',
                'CS': 'CSD',
    ...
    try {
        ...
        if (match.label !== 'unknown') {
            ...
            try {
                ...
                if (attendanceError.message.includes('already recorded') || attendanceError.message.includes('already marked')) {
                    setBackgroundState('face-verified');
                    setCameraFrameState('verified');
...
function showSuccessResult(student, confidence) {
    ...
    if (userMode === 'student') {
        setTimeout(() => {
            console.log('✅ Auto-redirecting student to classes-student.html');
            stopCamera();
...
window.goBack = function() {
    ...
    if (userMode === 'teacher') {
        ...
        if (department) {
            window.location.href = `${department.toLowerCase()}.html`;
        } else {
